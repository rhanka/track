...
export const SEAM_V0_SCHEMA = {
  ...
  $defs: {
    ...
    Violation: {
      description:
        'A scope.verification.violations[] entry is the deterministic JSON.stringify({severity,code,path,message}) projection (S2) — a display/index string track records VERBATIM and NEVER parses. The canonical detail is the full VerificationRun behind artifactLocator.',
      type: 'object',
    ...
    VerificationCheck: {
      description:
        'One check within a VerificationRun (per-CHECK target — one harness verify aggregates N checks across WPs/criteria). ≥1 target REQUIRED; a target-less check FAILS CLOSED at the adapter (never emitted).',
      type: 'object',
